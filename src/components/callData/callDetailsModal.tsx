...
const CallDetailsSheet: React.FC<CallDetailsSheetProps> = ({ isOpen, call, onClose }) => {
 ...
 useEffect(() => {
    if (status === "authenticated" && session) {
        console.log("Session data in CallDetailsSheet:", session);
      setUser(session?.memberEmail);
    }
  }, [session, status]);
    ...
    const getPartySpecificText = (user: string) => {
  if (user === "tvk@gmail.com") {
    return {
      partyName: "தமிழக வெற்றி கழகத்திலிருந்து",
      leaderName: "விஜய்",
      partyShort: "டிவிகே",
      partyFull: "தமிழக வெற்றி கழகம்"
    };
  } else if (user === "dmk@gmail.com") {
    return {
      partyName: "திராவிட முன்னேற்றக் கழகத்திலிருந்து",
      leaderName: "முத்துவேல் கருணாநிதி ஸ்டாலின்",
      partyShort: "டிஎம்கே", 
      partyFull: "திராவிட முன்னேற்றக் கழகம்"
    };
  } else {
    return {
      partyName: "தமிழக வெற்றி கழகத்திலிருந்து",
...
// Function to process individual text segments
const processTextSegment = (text: string, user: string) => {
  const partyText = getPartySpecificText(user);
  
  return text
    .replace(/தமிழக வெற்றி கழகத்திலிருந்து/g, partyText.partyName)
    .replace(/விஜய்/g, partyText.leaderName)
    .replace(/டிவிகே/g, partyText.partyShort)
    .replace(/தமிழக வெற்றி கழகம்/g, partyText.partyFull)
    .replace(/டிவிக்கே/g, partyText.partyShort);
};
                                                ...
                                                <div key={index} className="flex flex-col">
                                                    ...
                                                    <div className={`p-3 rounded-lg max-w-md ${segment.speaker === 'agent'
                                                        ...
                                                        {/* <p className="text-sm">{segment.text}</p> */}
                                                          <p className="text-sm">
              {processTextSegment(segment.text, user || "TVK")}
            </p>
                                                    </div>
                                                </div>
                                            ))}
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div className="border border-gray-200 p-4 rounded">
                                                ...
                                                <p className="font-mono text-sm text-gray-700">{call.callerNumber}</p>
                                            </div>
                                            ...
                                            <div className="border border-gray-200 p-4 rounded">
                                                ...
                                                <p className="text-sm text-gray-700 capitalize">{call.gender}</p>
                                            </div>
                                            ...
                                            <div className="border border-gray-200 p-4 rounded">
                                                ...
                                                <p className="text-sm font-semibold text-black">{call.overall_performance_rating}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="border border-gray-200 p-4 rounded">
                                                <h3 className="text-sm font-medium text-black mb-2">Interest Level</h3>
                                                <span className={`px-2 py-1 text-xs font-medium uppercase tracking-wide rounded ${call.property_buying_interest === 'high' ? 'bg-black text-white' :
                                                    call.property_buying_interest === 'medium' ? 'bg-gray-600 text-white' :
                                                        'bg-gray-400 text-white'
                                                    }`}>
                                                    {call.property_buying_interest}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="border border-gray-200 p-4 rounded bg-gray-50">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="text-right">
                                                    <div className="text-xs text-gray-600 mb-1">Emotional State</div>
                                                    <span className={`px-2 py-1 text-xs font-medium uppercase tracking-wide rounded ${call.emotional_state === 'positive' ? 'bg-black text-white' :
                                                        call.emotional_state === 'neutral' ? 'bg-gray-500 text-white' :
                                                            'bg-gray-400 text-white'
                                                        }`}>
                                                        {call.emotional_state}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="border border-gray-200 p-4 rounded">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 text-sm">
                                                <div>
                                                    ...
                                                    <span className="ml-2 text-gray-900">{call.timestamp}</span>
                                                </div>
                                                <div>
                                                    <span className="text-gray-600">Communication Quality:</span>
                                                    <span className="ml-2 text-gray-900">{call.communication_quality}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
